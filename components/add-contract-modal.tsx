...
export function AddContractModal({
  ...
  // ========== FIXED: Close sub‑modal after adding, keep main modal open ==========
  const handleAddOldService = () => {
    ...
    // CLOSE the sub‑modal (main modal stays open)
    setSubModal({
      open: false,
      startDate: '',
      endDate: '',
  ...
  // ---------- GUARD: prevent main modal from closing while sub‑modal is open ----------
  const handleMainModalOpenChange = (isOpen: boolean) => {
    // If the main modal is asked to close and the sub‑modal is still open, ignore it.
    if (!isOpen && subModal.open) {
      return
    }
    onOpenChange(isOpen)
  }
  ...
  return (
    <>
      ...
      {/* SUB‑MODAL FOR OLD SERVICES */}
      <Dialog
        open={subModal.open}
        onOpenChange={(isOpen) => setSubModal({ ...subModal, open: isOpen, errors: {} })}
      >
        <DialogContent
