...
export default function ViewQuotationPage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const safeStr = (val: any): string => String(val ?? '-')
      const safeNum = (val: any): string => String(Number(val ?? 0).toLocaleString('en-IN'))
      const safeDate = (val: any): string => val ? new Date(val).toLocaleDateString('en-IN') : '-'
      ...
      doc.text(safeStr(quotation.quotation_number), margin + 10, margin + 20)
      ...
      doc.text(safeStr(quotation.customer_name), margin, yPosition)
      yPosition += 5
      ...
      if (quotation.customer_address) {
        doc.setFontSize(8)
        doc.setTextColor(80, 80, 80)
        const lines = doc.splitTextToSize(`Address: ${safeStr(quotation.customer_address)}`, 80)
      ...
      const itemsData = (quotation.items ?? []).map((item) => [
        ...
        `Rs. ${safeNum(item.unit_price)}`,
        `Rs. ${safeNum(item.amount)}`,
      ])
      ...
      if (quotation.include_gst) {
        doc.setFontSize(9)
        doc.setTextColor(80, 80, 80)
        doc.text(`SGST (${safeStr(quotation.gst_rate ?? 0)}%):`, rightCol, yPosition)
        doc.setTextColor(0, 0, 0)
        doc.text(`Rs. ${sgst.toLocaleString('en-IN')}`, pageW - margin - 5, yPosition, { align: "right" })
        yPosition += 5

        doc.setTextColor(80, 80, 80)
        doc.text(`CGST (${safeStr(quotation.gst_rate ?? 0)}%):`, rightCol, yPosition)
        doc.setTextColor(0, 0, 0)
      ...
      if (grandTotal > 0) {
        ...
        const lines = doc.splitTextToSize(`Amount in words: ${safeStr(amountInWords)}`, pageW - 2 * margin - 10)
        doc.text(lines, margin, yPosition)
        yPosition += lines.length * 4 + 3
      }
      ...
      doc.text(
        `Generated on ${safeDate(new Date())} | Quotation ID: ${safeStr(quotation.id)}`,
        pageW / 2,
        pageH - 8,
        { align: "center" }
      )

      doc.save(`${safeStr(quotation.quotation_number)}.pdf`)
