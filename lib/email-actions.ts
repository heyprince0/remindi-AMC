...
/**
 * Server action to send team invite email
 * 
 ...
 * @param role - Role being offered (admin or member)
 * @param inviteLink - Link to accept the invite
 * @returns Object with success status and optional error message
 */
export async function triggerInviteMemberEmail(
  inviteeEmail: string,
  inviterName: string,
  businessName: string,
  role: string,
  ...
  try {
    if (!inviteeEmail || !inviterName || !businessName || !role || !inviteLink) {
      return {
        success: false,
    ...
    // Send via email service - using the generic send-email endpoint
    ...
    if (!response.ok) {
      ...
      console.error(`[Email Actions] Failed to send invite email: ${error.error}`)
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
    console.error(`[Email Actions] Exception in triggerInviteMemberEmail: ${errorMessage}`)
